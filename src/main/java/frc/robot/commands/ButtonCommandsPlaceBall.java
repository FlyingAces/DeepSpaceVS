...
public class ButtonCommandsPlaceBall extends CommandGroup {
	public ButtonCommandsPlaceBall(RobotMap.Controller controller) {
		switch(controller) {
		case TRIGGER_LB:
			...
			break;
		case TRIGGER_RB:
			addSequential(new ChangeHandStateCommand(RobotArmCalculations.HandState.PLACE_DISK));
		...
		case Y_BUTTON:
			...
			addSequential(new MoveArmWristToCommand(RobotMap.PLACE_START_X, RobotMap.BALL_HIGH_POSITION_Y));
		...
		case B_BUTTON:
			addSequential(
				new ConditionalCommand(
					...
					protected boolean condition() {
						return ((ArmSubsystem.getInstance().getWristTargetY() < RobotMap.PICK_UP_MID_STEP_Y) && 
								(RobotMap.BALL_MIDDLE_POSITION_Y > RobotMap.PICK_UP_MID_STEP_Y)) ||
			...
			addSequential(new MoveArmWristToCommand(RobotMap.PLACE_START_X, RobotMap.BALL_MIDDLE_POSITION_Y));
			break;
		case A_BUTTON:
			addSequential(
				new ConditionalCommand(
			...
			addSequential(new MoveArmWristToCommand(RobotMap.PLACE_START_X, RobotMap.BALL_LOW_POSITION_Y));
			break;
